...
function updateSelectedServer(serv){
    ...
    ConfigManager.save()
    server_selection_button.innerHTML = '\u2022 ' + (serv != null ? serv.getName() : '선택된 서버가 없음')
    if(getCurrentView() === VIEWS.settings){
    ...
    sysAEx.on('message', (m) => {
        ...
        if(m.context === 'validateJava'){
            if(m.result == null){
                ...
                setOverlayContent(
                    '호환되는 Java가<br>설치되지 않았습니다',
                    '게임을 실행하려면 64비트 Java 8이 필요합니다. 설치를 진행하시겠습니까? 설치를 진행하면 <a href="http://www.oracle.com/technetwork/java/javase/terms/license/index.html">Oracle의 라이센스</a>에 동의한 것으로 간주합니다.',
                    'Java 설치',
                    '수동으로 설치'
                )
                setOverlayHandler(() => {
                    setLaunchDetails('Java 다운로드 준비 중..')
                ...
                setDismissHandler(() => {
                    $('#overlayContent').fadeOut(250, () => {
                        ...
                        setOverlayContent(
                            '실행하기 위해서는<br>Java가 필요합니다',
                            '유효한 64비트 Java 8이 필요합니다.<br><br><a href="https://github.com/dscalzi/HeliosLauncher/wiki/Java-Management#manually-installing-a-valid-version-of-java">Java 관리 가이드</a>를 참조해 Java를 수동으로 설치하는 방법을 확인하세요.',
                            '확인',
                            '뒤로'
                        )
                        setOverlayHandler(() => {
            ...
            if(m.result === true){
                ...
                setOverlayContent(
                    ...
                    '자바를 설치하는 중에 문제가 발생했습니다. 수동으로 Java를 설치해야 합니다. <a href="https://github.com/dscalzi/HeliosLauncher/wiki">문제 해결 가이드</a>를 통해 자세한 내용과 방법을 확인하세요.',
                    '확인'
                )
                setOverlayHandler(() => {
                    toggleOverlay(false)
...
function dlAsync(login = true){
    ...
    aEx.on('error', (err) => {
        ...
        showLaunchFailure('실행 중 오류 발생', err.message || '콘솔(CTRL + Shift + i)을 통해 세부 정보를 확인하세요.')
    })
    aEx.on('close', (code, signal) => {
        if(code !== 0){
            loggerLaunchSuite.error(`AssetExec exited with code ${code}, assuming error.`)
            showLaunchFailure('실행 중 오류 발생', '콘솔(CTRL + Shift + i)을 통해 세부 정보를 확인하세요.')
        }
    })
        ...
        if(m.context === 'validate'){
            switch(m.data){
                ...
                case 'version':
                    ...
                    setLaunchDetails('에셋 무결성 확인 중..')
                    break
                case 'assets':
                    setLaunchPercentage(60, 100)
                    loggerLaunchSuite.log('Asset Validation Complete')
                    setLaunchDetails('라이브러리 무결성 확인 중..')
                    break
                case 'libraries':
            ...
            switch(m.data){
                case 'download':
                    ...
                    if(m.error.code === 'ENOENT'){
                        showLaunchFailure(
                            '다운로드 오류',
                            '파일 서버에 연결할 수 없습니다. 인터넷 연결을 확인한 후 다시 시도해주세요.'
                        )
                    } else {
                        showLaunchFailure(
                            '다운로드 오류',
                            '콘솔(CTRL + Shift + i)을 통해 세부 정보를 확인하세요. 다시 시도해주세요.'
                        )
                    }
            ...
            if(login && allGood) {
                ...
                const gameStateChange = function(data){
                    ...
                    if(SERVER_JOINED_REGEX.test(data)){
                        DiscordWrapper.updateDetails('모드팩의 세계로!')
                    } else if(GAME_JOINED_REGEX.test(data)){
                        DiscordWrapper.updateDetails('HAYO!')
                    }
                }
                ...
                const gameErrorListener = function(data){
                    ...
                    if(data.indexOf('Could not find or load main class net.minecraft.launchwrapper.Launch') > -1){
                        ...
                        showLaunchFailure('실행 중 오류 발생', '메인 파일인 LaunchWrapper를 제대로 다운로드하지 못하여 게임을 실행할 수 없습니다.<br><br>문제를 해결하려면 안티바이러스 프로그램을 잠시 종료한 뒤 게임을 다시 실행해주세요.<br><br>시간이 있다면 <a href="https://github.com/dscalzi/HeliosLauncher/issues">문제를 보고하여</a> 사용하고 있는 안티바이러스 프로그램이 무엇인지 알려주세요. 문제를 해결할 수 있도록 노력하겠습니다.')
                    }
                }
                ...
                try {
                    ...
                    setLaunchDetails('완료. 즐거운 시간 보내세요!')
    ...
    refreshDistributionIndex(true, (data) => {
        ...
        refreshDistributionIndex(false, (data) => {
            ...
            if(DistroManager.getDistribution() == null){
                showLaunchFailure('치명적인 오류', 'Distribution index의 복사본을 불러올 수 없습니다. 콘솔(CTRL + Shift + i)을 통해 세부 정보를 확인하세요.')
