    ...
    Array.from(document.getElementsByClassName('settingsAuthAccountLogOut')).map((val) => {
        val.onclick = (e) => {
            ...
            if(Object.keys(ConfigManager.getAuthAccounts()).length === 1){
                ...
                setOverlayContent(
                    '경고<br>마지막 남은 계정입니다',
                    '런처를 이용하기 위해서는 최소 하나 이상의 계정에 로그인해야 합니다. 로그아웃 후 런처를 실행하면 로그인이 필요합니다.<br><br>정말 로그아웃할까요?',
                    '로그아웃',
                    '취소'
                )
                setOverlayHandler(() => {
        ...
        authAccountStr += `<div class="settingsAuthAccount" uuid="${acc.uuid}">
            ...
            <div class="settingsAuthAccountRight">
                ...
                <div class="settingsAuthAccountActions">
                    <button class="settingsAuthAccountSelect" ${selectedUUID === acc.uuid ? 'selected>선택된 계정 &#10004;' : '>선택된 계정'}</button>
    ...
    Array.from(sEls).map((v, index, arr) => {
        v.onclick = () => {
            ...
            if(res){
                ...
                setOverlayContent(
                    `${fullName} 모드를<br>제거하지 못했습니다`,
                    '파일이 사용 중인지 확인한 후 다시 시도해주세요.',
                    '확인'
                )
    ...
    jg._validateJavaBinary(execPath).then(v => {
        if(v.valid){
            ...
            if(v.version.major < 9) {
                settingsJavaExecDetails.innerHTML = `선택: Java ${v.version.major} Update ${v.version.update} (x${v.arch})${vendor}`
            } else {
                settingsJavaExecDetails.innerHTML = `선택: Java ${v.version.major}.${v.version.minor}.${v.version.revision} (x${v.arch})${vendor}`
            }
        } else {
            settingsJavaExecDetails.innerHTML = '유효하지 않은 선택'
        }
    })
}
...
function populateSettingsUpdateInformation(data){
    if(data != null){
        settingsUpdateTitle.innerHTML = `사용 가능한 새로운 ${isPrerelease(data.version) ? '프리릴리즈' : '릴리즈'}가 있습니다`
        ...
        if(process.platform === 'darwin'){
            settingsUpdateButtonStatus('Github에서 다운로드<span style="font-size: 10px;color: gray;text-shadow: none !important;">런처를 닫고 exe를 실행하여 업데이트하세요.</span>', false, () => {
        ...
        settingsUpdateTitle.innerHTML = '런처가 최신 버전입니다'
